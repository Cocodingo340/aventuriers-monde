...
public class Joueur {
                ...
                for (Route r : jeu.getRoutesLibres()) {
                    if (r.getNom().equals(choixFinal)) {
                        if(r.getClass().equals(RouteTerrestre.class)) {
                            ...
                            if (peutPayerRoutePaire(r)){
                                choixValide = true;
                                PrendreRoutePaire(r);
                            }

                            else{
                                log("Vous ne pouvez pas poser cette route paire");
                                break;
                            }
                        }
                    }
    ...
    public boolean peutPayerRoutePaire(Route route){
        ...
        if(cmbDePaire>=prix && nbPionsWagon>=prix){
            return true;
        }
        return false;
    }
    ...
    public void PrendreRoutePaire(Route route){
        ...
        while (CouleurBesoinJoker==null) {
            for (Couleur c : Couleur.values()) {
                if (compteurDeCarteWagonDeCouleurPlusJoker(route, c, copieCarteTransport) == 2) {
        ...
        while (prix>0) {
            ...
            for (CarteTransport c : copieCarteTransport) {
                if (c.getNom().equals(choix)) {
                    if (c.getType().equals(TypeCarteTransport.WAGON)) {
                        if (compteurDeCarteWagonDeCouleurPlusJoker(route, c.getCouleur(),copieCarteTransportMaisComplete) >= 2) {
